...
@section('content')
    @php
        $isEditable = !$log->mark_as_complete && $role == 'supervisor' && !$isLocked;
    @endphp
        ...
        <div class="card-body">
            <form id="jobUpdateForm" enctype="multipart/form-data">
                    ...
                    <div class="col-md-3">
                        ...
                        @if ($role == 'supervisor')
                            <input type="text" name="wo_number" class="form-control" value="{{ $log->wo_number }}"
                                {{ !$isEditable || $log->is_excel_upload ? 'readonly' : '' }}>
                        @else
                            <input type="text" name="wo_number" class="form-control" value="{{ $log->wo_number }}"
                    ...
                    <div class="col-md-6">
                        ...
                        @if ($role == 'supervisor')
                            <textarea class="form-control" name="work_description" rows="3"
                                {{ !$isEditable || $log->is_excel_upload ? 'readonly' : '' }}>{{ $log->work_description }}</textarea>
                        @else
                            <textarea class="form-control" name="work_description" rows="3"
                    ...
                    <div class="col-md-6">
                        ...
                        @if ($role == 'supervisor')
                            <textarea class="form-control" name="asset_description" rows="3"
                                {{ !$isEditable || $log->is_excel_upload ? 'readonly' : '' }}>{{ $log->asset_description }}</textarea>
                        @else
                            <textarea class="form-control" name="asset_description" rows="3"
                ...
                <div class="mt-4">
                    ...
                    @if ($role == 'supervisor')
                        <textarea class="form-control" name="supervisor_notes" rows="4" {{ $isEditable ? '' : 'readonly' }}>{{ $log->supervisor_notes }}</textarea>
                    @else
                    ...
                    success: function(response) {
                        ...
                        setTimeout(() => {
                            window.location.href =
                                `{{ route('supervisors-shift-log.index', ['role' => $role]) }}?date=${response.date}`;
                        }, 1000);
                    },
